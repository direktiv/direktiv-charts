...
curl -L -H 'Cache-Control: no-cache' https://github.com/knative/serving/releases/download/knative-v1.1.0/serving-crds.yaml > $dir/crds/serving-crds.yaml
curl -L -H 'Cache-Control: no-cache' https://github.com/knative/serving/releases/download/knative-v1.1.0/serving-core.yaml > $dir/templates/serving-core.yaml

# knative uses {{ }} in comments, disable them
sed -i 's/{{/{{ "{{" }}/g' $dir/templates/serving-core.yaml

# delete namespace
sed -i '1,24d' $dir/templates/serving-core.yaml
